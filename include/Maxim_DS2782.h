...
private:
  ...
  static constexpr float VOLTAGE_FACTOR = 5 / (float)((1 << 10) << 5);
  ...
  float transferVoltage(int16_t raw_value, HardwareSerial *SerialMon) {
    // SerialMon->println(-(float)raw_value * VOLTAGE_FACTOR);
    return (float)raw_value * VOLTAGE_FACTOR;
  }

  // Convert raw register value for TEMP and return in °C.
  float transferTemperature(int16_t raw_value, HardwareSerial *SerialMon) {
    //SerialMon->println((float)raw_value * TEMPERATURE_FACTOR);
    return (float)raw_value * TEMPERATURE_FACTOR;
  }

  // Convert raw register value for CURRENT, IAVG and return in mA.
  float transferCurrent(int16_t raw_value, HardwareSerial *SerialMon) {
    //SerialMon->printf("mAh: %.3f", ((float)raw_value * CURRENT_FACTOR) / Rsns_ohm);

    //SerialMon->println(((float)raw_value * CURRENT_FACTOR) / Rsns_ohm);
  ...
  int16_t read_int16(uint8_t register_address) {
   ...
   int16_t read_int16a(uint8_t register_address, HardwareSerial *SerialMon) {
    bus->beginTransmission(i2c_address);
    bus->write(register_address);
    bus->endTransmission();
  ...
  uint16_t read_uint16a(uint8_t register_address, HardwareSerial SerialMon) {
    bus->beginTransmission(i2c_address);
    bus->write(register_address);
    bus->endTransmission();
...
public:
  Maxim_DS2782(TwoWire *bus, uint8_t i2c_address, float Rsns_ohm, HardwareSerial *SerialMon) {
    this->bus = bus;
    this->SerialMon = SerialMon;
  ...
  float readTemperature() {
    //SerialMon->printf("Temp: %d", read_int16a(Registers::Register_TEMP, SerialMon));
    return transferTemperature(read_int16a(Registers::Register_TEMP, SerialMon), SerialMon);
  }

  float readVoltage() {
    //SerialMon->println(-transferVoltage(read_int16a(Registers::Register_VOLT, SerialMon), SerialMon));
    return -transferVoltage(read_int16a(Registers::Register_VOLT, SerialMon), SerialMon);
  }
